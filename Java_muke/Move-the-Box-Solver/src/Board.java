public class Board {
    ...
    public Board(String[] lines) {
        ...
        for (int i = 0; i < N; i++) {
            if (lines[i].length() != M) {
                throw new IllegalArgumentException("Lines' length must be same M");
            }
            for (int j = 0; j < M; j++) {
    ...
    public char getData(int x, int y) {
        if (!inArea(x, y)) {
            throw new IllegalArgumentException("x or y out of index in function");
        }
        return data[x][y];
    }
    ...
    private static int d[][] = {{0, 1}, {1, 0}};//下、右
            ...
            for (int y = 0; y < M; y++) {
                if (data[x][y] != EMPTY) {
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (inArea(newX1, newY1) && inArea(newX2, newY2) && data[newX1][newY1] == data[x][y] && data[newX2][newY2] == data[x][y]) {
                            tag[x][y] = true;
